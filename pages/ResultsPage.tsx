...
export const ResultsPage: React.FC<ResultsPageProps> = ({ setCurrentPage, analysisResults, summary, currentUser, healthData, capturedImage, isPaymentComplete }) => {
  ...
  }, [isPaymentComplete, fetchEyeTips]);


  if (!analysisResults) {
    return (
      <PageContainer title={t('results_title')}>
        <LoadingSpinner text={t('results_loadingSummary')} />
      </PageContainer>
    );
  }
  ...
  return (
    <PageContainer title={t('results_title')} className="max-w-3xl mx-auto">
      {/* Hidden component for PDF generation, only mounted when needed */}
      {isPaymentComplete && (
        <div style={{ position: 'absolute', left: '-9999px', top: 0 }}>
          <ReportContents
            ref={reportRef}
            user={currentUser}
            questionnaireAnswers={healthData}
        ...
        {isPaymentComplete ? (
          <>
            ...
            <div className="mb-8 p-4 bg-accent/10 border-l-4 border-accent rounded-r-lg">
                ...
                <p className="text-primary leading-relaxed whitespace-pre-wrap">{summary || t('results_loadingSummary')}</p>
            </div>

            {/* Detailed Findings Section */}
            {analysisResults.length > 0 && (
              <div className="mb-8">
                <h3 className="text-xl font-semibold text-primary mb-4">{t('results_detailedFindingsTitle')}</h3>
            ...
            <div className="mb-8 p-4 bg-primary/5 border border-primary/20 rounded-lg">
              <h3 className="font-semibold text-lg text-primary flex items-center mb-3"><LightbulbIcon className="w-6 h-6 mr-2 text-primary" /> {t('results_generalEyeHealthTipsTitle')}</h3>
          ...
          <div className="text-center py-10">
            <h2 className="text-2xl font-bold text-primary mb-2">{t('payment_unlock_title')}</h2>
            <p className="text-primary/80 mb-6">{t('payment_unlock_description')}</p>
