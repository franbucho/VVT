...
export const FeedbackModal: React.FC<FeedbackModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" onClick={handleClose}>
      <div className="bg-white rounded-lg shadow-xl p-6 sm:p-8 max-w-md w-full transform transition-all" onClick={(e) => e.stopPropagation()}>
        {isSubmitted ? (
            <div className="text-center py-8">
                <CheckCircleIcon className="w-16 h-16 text-success mx-auto mb-4" />
                <h2 className="text-2xl font-bold text-primary-dark mb-2">{t('feedback_thanks_title')}</h2>
                <p className="text-primary-dark/80">{t('feedback_thanks_message')}</p>
            </div>
        ) : (
            <>
                <h2 className="text-2xl font-bold text-primary-dark mb-2">{t('feedback_title')}</h2>
                <p className="text-sm text-primary-dark/80 mb-6">{t('feedback_subtitle')}</p>
                ...
                <div className="flex justify-center items-center mb-6" aria-label="Rate your experience">
                  {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                      <StarIcon className={`w-10 h-10 cursor-pointer transition-all duration-150 transform hover:scale-110 ${star <= (hoverRating || rating) ? 'text-yellow-400' : 'text-gray-300'}`} />
                    </button>
                  ))}
                </div>
                ...
                <textarea
                  ...
                  className="w-full p-3 border border-gray-300 rounded-md h-24 focus:ring-2 focus:ring-accent focus:border-accent transition-shadow"
